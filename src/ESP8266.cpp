...
void ESP8266::initializeESP8266()
{
...
void ESP8266::displayConnectInfo()
{
...
void ESP8266::serverDemo(int inputData[4])
{
    int waterTemp = inputData[0];
    int waterLevel = inputData[1];
    int airTemp = inputData[2];
    int humidity = inputData[3];
        ...
        while (client.connected())
        {
            if (client.available())
            {
                ...
                if (c == '\n' && currentLineIsBlank)
                {
                    ...
                    htmlBody += F("Water Temperature (F): ");
                    htmlBody += String(waterTemp);
                    htmlBody += F("<br>\n");
                    htmlBody += F("Water Level (cm): ");
                    htmlBody += String(waterLevel);
                    htmlBody += F("<br>\n");
                    htmlBody += F("Air Temperature (F): ");
                    htmlBody += String(airTemp);
                    htmlBody += F("<br>\n");
                    htmlBody += F("Humidity %: ");
...
void ESP8266::errorLoop(int error)
{
...
void ESP8266::serialTrigger(String message)
{
    Serial.println();
